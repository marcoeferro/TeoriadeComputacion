...
public class Converter {
	...
	public AFN textToAFN(List<String> texto) {
		...
		for(String linea : textoFiltrado) {
			if(linea.split("->")[0].equalsIgnoreCase("inic")) this.parserInicial(linea);
			else if(this.esTransicion(linea)) this.parserTransicion(linea);
			else if(this.esEstadoFinal(linea)) this.parserFinal(linea);
		}
		a = this.automata;
		this.automata = new AFN();
		return a;
